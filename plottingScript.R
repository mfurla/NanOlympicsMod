...
#hits number and metagene plots
  ...
  plot_type <- c("tx", "mrna", "tx")
  names(plot_type) <- c("yeast", "mESC", "oligos")

  #plot reference set metagene
  pdf(paste0("/path/to/", dataset, "_gold_standard_peaks_metagene.pdf"))
  gs_file_list <- as.list(tmp_file_gs)
  gs_file_list_plat <- c("refSet_m6A", "refSet_MAZTER-seq_m6A-seq", "refSet_miCLIP2_GLORI")
  names(gs_file_list_plat) <- c("oligos", "yeast", "mESC")
  ...
  txpromoterLength_val = c(1, 1000, 1000)
  txtailLength_val = c(1, 1000, 1000)
  names(txpromoterLength_val) <- c("oligos", "yeast", "mESC")
  names(txtailLength_val) <- c("oligos", "yeast", "mESC")
  ...
  colors_MP[11] <- "gold"
  tools <- c("Nanocompore", "Tombo", "xPore", "Yanocomp", "ELIGOS", "EpiNano-Error", "DiffErr", "DRUMMER", "m6Anet", "DENA", "EpiNano-SVM", "MINES")
...
#plot barplots with the number of hits for each tool
  ...
  num_hits_df <- data.frame(Tool = c(names(num_filt_peaks_oligos), names(num_filt_peaks_yeast), names(num_filt_peaks_mESC)), Dataset = c(rep("Oligos", length(num_filt_peaks_oligos)), rep("Yeast", length(num_filt_peaks_yeast)), rep("mESC", length(num_filt_peaks_mESC))), Num_hits = c(num_filt_peaks_oligos, num_filt_peaks_yeast, num_filt_peaks_mESC))
...
###############################
#False Positive bins motif enrichment analysis
...
#plot running time
  ...
  ggplot(oligos_running_time, aes(x = tool, y = time_h, fill = analysis_type, group = analysis_type)) +
    ...
    theme(legend.title = element_text(size = 15),
          ...
          panel.background = element_blank())
  ggsave("/path/to/Running_time_oligos.pdf")  
  ...
  ggplot(yeast_running_time, aes(x = tool, y = time_h, fill = analysis_type, group = analysis_type)) +
    ...
    theme(legend.title = element_text(size = 15),
          ...
          axis.title.y = element_text(size = 15),
          panel.background = element_blank())
  ggsave("/path/to/Running_time_yeast.pdf")  
  ...
  ggplot(mESC_running_time, aes(x = tool, y = time_h, fill = analysis_type, group = analysis_type)) +
    ...
    theme(legend.title = element_text(size = 15),
          ...
          axis.title.y = element_text(size = 15),
          panel.background = element_blank())
  ggsave("/path/to/Running_time_mESC.pdf")  